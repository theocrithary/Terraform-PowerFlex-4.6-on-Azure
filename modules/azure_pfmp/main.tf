...
resource "azurerm_network_interface" "storage_instance_nic" {
  ...
  location                       = data.azurerm_resource_group.pflex_rg.location
  resource_group_name            = data.azurerm_resource_group.pflex_rg.name
...
resource "azurerm_linux_virtual_machine" "storage_instance" {
  ...
  provisioner "local-exec" {
    command = count.index > 2 ? "whoami" : <<-EOT
      az disk update -n ${var.prefix}-vm-${count.index}-os-disk -g ${data.azurerm_resource_group.pflex_rg.name} --set tier=P40 --no-wait
      az disk wait --updated -n ${var.prefix}-vm-${count.index}-os-disk -g ${data.azurerm_resource_group.pflex_rg.name}
    EOT
  }
}
